...
// constructor of ExamDetails
    ExamDetails::ExamDetails(int course_number, int month, int day, double starting_time, int duration_time,
                             std::string zoom_link)
    {
        ...
        if ((std::abs(ceil(starting_time) - starting_time - half) < epsilon) &&
            (std::abs(starting_time - ceil(starting_time)) < epsilon))
        {
